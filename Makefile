...
CFLAGS+=$(shell ${PKG_CONFIG} --cflags x11 xinerama gdk-pixbuf-xlib-2.0 )
LDADD+=$(shell ${PKG_CONFIG} --libs x11 xinerama  gdk-pixbuf-xlib-2.0 )

ifeq (${LDADD},${EMPTY})
$(error Failed to find the required dependencies: x11, xinerama, gdk-pixbuf-xlib-2.0)
endif
